...
npm run resume:pdf # Generate public/Ron_Rounsifer_Resume.pdf from the Markdown source
...
**Résumé source of truth.** `Ron_Rounsifer_Resume.md` is canonical. The static
`/resume/` route reads it at build time for its responsive HTML fallback, and
`scripts/generate-resume-pdf.mjs` generates the desktop-embedded/downloadable
`public/Ron_Rounsifer_Resume.pdf`. The PDF is ignored because `predev` and
`prebuild` regenerate it. Do not edit or commit the generated PDF directly.
...
- `resume/page.tsx` → static résumé route with an embedded desktop PDF and responsive HTML on smaller screens.
